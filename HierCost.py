...
def train_HierCost(input_dir, num_features, classifier_names, mode, output_dir=None):
    ...
    for classifier_name in classifier_names:
        ...
        # if os.path.exists(data_file):
            if "HierCost_LR" in classifier_name:
                command = "python HierCost/train.py -t %s -d %s -m %s -f %d -r 0.01 -c lr -i > %s" % (
                    hierarchy_file, data_file, model_dir, num_features, log_file)
                logger.info(command)
                # pool.apply_async(os.system, args=(command,))
                os.system(command)
            elif "HierCost_TrD" in classifier_name:
                command = "python HierCost/train.py -t %s -d %s -m %s -f %d -r 0.01 -c trd -i > %s" % (
                    hierarchy_file, data_file, model_dir, num_features, log_file)
                logger.info(command)
                # pool.apply_async(os.system, args=(command,))
                os.system(command)
            elif "HierCost_NCA" in classifier_name:
                command = "python HierCost/train.py -t %s -d %s -m %s -f %d -r 0.01 -c nca -i > %s" % (
                    hierarchy_file, data_file, model_dir, num_features, log_file)
                logger.info(command)
                # pool.apply_async(os.system, args=(command,))
                os.system(command)
            elif "HierCost_ExTrD" in classifier_name:
                command = "python HierCost/train.py -t %s -d %s -m %s -f %d -r 0.01 -c etrd -i > %s" % (
                    hierarchy_file, data_file, model_dir, num_features, log_file)
                logger.info(command)
                # pool.apply_async(os.system, args=(command,))
                os.system(command)
    ...
    for i in range(len(model_dirs)):
        if os.path.exists(model_dirs[i]):
            if not os.path.exists(pred_files[i]):
                command = "python HierCost/predict.py -t %s -d %s -m %s -f %d -p %s > %s" % (
                    hierarchy_file, test_file, model_dirs[i], num_features, pred_files[i], metrics_files[i])
                logger.info(command)
                os.system(command)
...
def main(input_dir=settings.data_dir_20ng, label_ratio=0.1, times=1, classifier_names=None):
    ...
    if not classifier_names:
        classifier_names = ['HierCost_LR', 'HierCost_TrD', 'HierCost_NCA', 'HierCost_ExTrD']
    if label_ratio == 1.0:
        times = 1
    for mode in ["labeled", "dataless"]:
        ...
        with open(os.path.join(input_dir, str(label_ratio), 'HierCost_%s.csv' % (mode)), 'w') as f:
...
if __name__ == "__main__":
    log_filename = os.path.join(settings.log_dir, 'HierCost.log')
    logconfig.logging.config.dictConfig(logconfig.logging_config_dict('INFO', log_filename))

    pool = Pool(10)
    classifier_names = ['HierCost_LR', 'HierCost_ExTrD']
